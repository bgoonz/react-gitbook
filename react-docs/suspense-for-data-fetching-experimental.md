...
* [What Is Suspense, Exactly?](broken-reference)
  * [What Suspense Is Not](broken-reference)
  * [What Suspense Lets You Do](broken-reference)
* [Using Suspense in Practice](broken-reference)
  * [What If I Don’t Use Relay?](broken-reference)
  * [For Library Authors](broken-reference)
* [Traditional Approaches vs Suspense](broken-reference)
  * [Approach 1: Fetch-on-Render (not using Suspense)](broken-reference)
  * [Approach 2: Fetch-Then-Render (not using Suspense)](broken-reference)
  * [Approach 3: Render-as-You-Fetch (using Suspense)](broken-reference)
* [Start Fetching Early](broken-reference)
  * [We’re Still Figuring This Out](broken-reference)
* [Suspense and Race Conditions](broken-reference)
  * [Race Conditions with useEffect](broken-reference)
  * [Race Conditions with componentDidUpdate](broken-reference)
  * [The Problem](broken-reference)
  * [Solving Race Conditions with Suspense](broken-reference)
* [Handling Errors](broken-reference)
* [Next Steps](broken-reference)

### What Is Suspense, Exactly? <a href="#what-is-suspense-exactly" id="what-is-suspense-exactly"></a>
...
This demo is a teaser. Don't worry if it doesn't quite make sense yet. We'll talk more about how it works below. Keep in mind that Suspense is more of a _mechanism_, and particular APIs like `fetchProfileData()` or `resource.posts.read()` in the above example are not very important. If you're curious, you can find their definitions right in the [demo sandbox](https://codesandbox.io/s/frosty-hermann-bztrp).

Suspense is not a data fetching library. It's a **mechanism for data fetching libraries** to communicate to React that _the data a component is reading is not ready yet_. React can then wait for it to be ready and update the UI. At Facebook, we use Relay and its [new Suspense integration](https://relay.dev/docs/getting-started/step-by-step-guide/). We expect that other libraries like Apollo can provide similar integrations.
...
#### What Suspense Is Not <a href="#what-suspense-is-not" id="what-suspense-is-not"></a>
...
#### What Suspense Lets You Do <a href="#what-suspense-lets-you-do" id="what-suspense-lets-you-do"></a>
...
* **It helps you avoid race conditions.** Even with `await`, asynchronous code is often error-prone. Suspense feels more like reading data _synchronously_ — as if it were already loaded.

### Using Suspense in Practice <a href="#using-suspense-in-practice" id="using-suspense-in-practice"></a>
...
Although it's technically doable, Suspense is **not** currently intended as a way to start fetching data when a component renders. Rather, it lets components express that they're "waiting" for data that is _already being fetched_. **Building Great User Experiences with Concurrent Mode and Suspense describes why this matters and how to implement this pattern in practice.**
...
### Traditional Approaches vs Suspense <a href="#traditional-approaches-vs-suspense" id="traditional-approaches-vs-suspense"></a>
...
#### Approach 1: Fetch-on-Render (not using Suspense) <a href="#approach-1-fetch-on-render-not-using-suspense" id="approach-1-fetch-on-render-not-using-suspense"></a>
...
If fetching user details takes three seconds, we'll only _start_ fetching the posts after three seconds! That's a "waterfall": an unintentional _sequence_ that should have been parallelized.
...
#### Approach 2: Fetch-Then-Render (not using Suspense) <a href="#approach-2-fetch-then-render-not-using-suspense" id="approach-2-fetch-then-render-not-using-suspense"></a>
...
We've solved the previous network "waterfall", but accidentally introduced a different one. We wait for _all_ data to come back with `Promise.all()` inside `fetchProfileData`, so now we can't render profile details until the posts have been fetched too. We have to wait for both.
...
#### Approach 3: Render-as-You-Fetch (using Suspense) <a href="#approach-3-render-as-you-fetch-using-suspense" id="approach-3-render-as-you-fetch-using-suspense"></a>
...
**With Suspense, we don't wait for the response to come back before we start rendering.** In fact, we start rendering _pretty much immediately_ after kicking off the network request:
...
Note how we eliminated the `if (...)` "is loading" checks from our components. This doesn't only remove boilerplate code, but it also simplifies making quick design changes. For example, if we wanted profile details and posts to always "pop in" together, we could delete the `<Suspense>` boundary between them. Or we could make them independent from each other by giving each _its own_ `<Suspense>` boundary. Suspense lets us change the granularity of our loading states and orchestrate their sequencing without invasive changes to our code.

### Start Fetching Early <a href="#start-fetching-early" id="start-fetching-early"></a>

If you're working on a data fetching library, there's a crucial aspect of Render-as-You-Fetch you don't want to miss. **We kick off fetching **_**before**_** rendering.** Look at this code example closer:
...
### Suspense and Race Conditions <a href="#suspense-and-race-conditions" id="suspense-and-race-conditions"></a>
...
#### Race Conditions with `useEffect` <a href="#race-conditions-with-useeffect" id="race-conditions-with-useeffect"></a>
...
#### Race Conditions with `componentDidUpdate` <a href="#race-conditions-with-componentdidupdate" id="race-conditions-with-componentdidupdate"></a>
...
#### Solving Race Conditions with Suspense <a href="#solving-race-conditions-with-suspense" id="solving-race-conditions-with-suspense"></a>
...
When we write code with Promises, we might use `catch()` to handle errors. How does this work with Suspense, given that we don't _wait_ for Promises to start rendering?
...
It would catch both rendering errors _and_ errors from Suspense data fetching. We can have as many error boundaries as we like but it's best to [be intentional](https://aweary.dev/fault-tolerance-react/) about their placement.
...
* If we intentionally _want_ to show an inconsistent UI for a small period of time, can we do that?
